...
for (const file of [
  ...
  'robots.txt', 'llms.txt', 'site.webmanifest', '_redirects', '_headers'
]) copyFile(file);

// Root-level editorial roundup pages generated by daily-publish.js.
for (const slug of editorialSlugs) copyFile(`${slug}.html`);

// IndexNow ownership proof: https://foidslop.com/<key>.txt
fs.writeFileSync(path.join(OUTPUT, `${KEY}.txt`), KEY);
...
for (const file of fs.readdirSync(path.join(ROOT, 'assets', 'fonts'))) {
  copyFile(path.join('assets', 'fonts', file), path.join('fonts', file));
}
...
if (fs.existsSync(path.join(ROOT, 'slop', 'archive'))) copyDirectory(path.join('slop', 'archive'));
