...
function renderDictionaryIndex() {
  ...
  return `${commonHead({ route, title: 'The Slop Dictionary: Internet Slang Explained', description: 'Internet vocabulary nobody should reasonably need explained: foidslop, foid, femoid, mog, looksmaxxing, AI slop, girl dinner, and more.', schema })}${header(route, 'dictionary')}<main id="main" class="culture-index"><p class="content-eyebrow">Reference desk / internet vocabulary</p><h1>The Slop Dictionary</h1><p class="article-deck">Internet language moves faster than dignity. This is the field guide.</p><div class="dictionary-list">${entries.map(entry => `<a href="${dictionaryRoute(entry).startsWith('dictionary/') ? entry.slug : `../${dictionaryRoute(entry)}`}" class="dictionary-row"><strong>${esc(entry.term)}</strong><span>${esc(entry.definition)}</span></a>`).join('')}</div></main>${footer(route)}`;
}
...
function renderCultureIndex() {
  const route = 'culture';
  const schema = { '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'foidslop Culture', description: 'Field notes from internet culture.', url: canonical(route), hasPart: culture.articles.map(article => ({ '@type': 'Article', headline: article.title, url: `${BASE_URL}/culture/${article.slug}` })) };
  return `${commonHead({ route, title: 'Culture: Foidslop, Slang and Internet Taxonomy', description: 'Foidslop media, usernames, slop vocabulary, girl dinner, internet taxonomy, and other extremely online subjects.', schema })}${header(route, 'culture')}<main id="main" class="culture-index"><p class="content-eyebrow">The culture desk</p><h1>Field notes from internet culture.</h1><p class="article-deck">Food remains the daily product. This is where the lore goes.</p><div class="culture-feature-grid">${culture.articles.map((article, index) => `<a class="culture-feature${index === 0 ? ' culture-feature--lead' : ''}" href="${article.slug}"><span>${esc(article.eyebrow.replace('Field notes / ', ''))}</span><strong>${esc(article.title)}</strong><p>${esc(article.deck)}</p></a>`).join('')}<a class="culture-feature" href="slop-index"><span>classification desk</span><strong>The Foidslop Media Index</strong><p>Extremely serious measurements of foid density, yearning, Tumblr residue, and other peer-reviewed metrics.</p></a></div><section class="culture-index-dictionary"><p class="content-eyebrow">Need a word explained?</p><h2>The Slop Dictionary</h2><p>Mog. Femoid. AI slop. Looksmaxxing. Vocabulary from forums, feeds, fandoms, and the wider timeline.</p><a href="../dictionary">Open the dictionary</a></section></main>${footer(route)}`;
}
