...
function relatedDictionary(entry, route) {
  ...
  return `<section class="culture-related"><p class="content-eyebrow">Related cursed vocabulary</p><div class="culture-card-grid">${items.map(item => `<a class="culture-card" href="${dictionaryHref(item.slug, route)}"><span>${esc(item.term)}</span><strong>${esc(item.title)}</strong><p>${esc(item.definition)}</p></a>`).join('')}</div></section>`;
}
...
function renderDictionaryIndex() {
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'DefinedTermSet', name: 'The Slop Dictionary', description: 'Internet vocabulary nobody should reasonably need explained.', url: canonical(route), hasDefinedTerm: entries.map(entry => ({ '@type': 'DefinedTerm', name: entry.term, url: canonical(dictionaryRoute(entry)) })) };
  return `${commonHead({ route, title: 'The Slop Dictionary | foidslop', description: 'Internet vocabulary nobody should reasonably need explained: foidslop, foid, femoid, mog, looksmaxxing, AI slop, girl dinner, and more.', schema })}${header(route, 'dictionary')}<main id="main" class="culture-index"><p class="content-eyebrow">Reference desk / unfortunate vocabulary</p><h1>The Slop Dictionary</h1><p class="article-deck">Internet language moves faster than dignity. This is the field guide.</p><div class="dictionary-list">${entries.map(entry => `<a href="${dictionaryRoute(entry).startsWith('dictionary/') ? entry.slug : `../${dictionaryRoute(entry)}`}" class="dictionary-row"><strong>${esc(entry.term)}</strong><span>${esc(entry.definition)}</span></a>`).join('')}</div></main>${footer(route)}`;
}
function cultureRelated(article) {
  const items = culture.articles.filter(candidate => candidate.slug !== article.slug).slice(0, 3);
  return `<section class="culture-related"><p class="content-eyebrow">Keep poisoning your browser history</p><div class="culture-card-grid">${items.map(item => `<a class="culture-card" href="${item.slug}"><span>${esc(item.eyebrow.replace('Field notes / ', ''))}</span><strong>${esc(item.title)}</strong><p>${esc(item.deck)}</p></a>`).join('')}</div></section>`;
}
...
function renderCultureIndex() {
  const route = 'culture';
  const schema = { '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'foidslop Culture', description: 'Field notes from the poisoned internet.', url: canonical(route), hasPart: culture.articles.map(article => ({ '@type': 'Article', headline: article.title, url: `${BASE_URL}/culture/${article.slug}` })) };
  return `${commonHead({ route, title: 'Culture | foidslop', description: 'Field notes from the poisoned internet: foidslop media, usernames, slop vocabulary, girl dinner, internet taxonomy, and other terminally online subjects.', schema })}${header(route, 'culture')}<main id="main" class="culture-index"><p class="content-eyebrow">The culture desk</p><h1>Field notes from the poisoned internet.</h1><p class="article-deck">Food remains the daily product. This is where the lore goes.</p><div class="culture-feature-grid">${culture.articles.map((article, index) => `<a class="culture-feature${index === 0 ? ' culture-feature--lead' : ''}" href="${article.slug}"><span>${esc(article.eyebrow.replace('Field notes / ', ''))}</span><strong>${esc(article.title)}</strong><p>${esc(article.deck)}</p></a>`).join('')}<a class="culture-feature" href="slop-index"><span>classification desk</span><strong>The Foidslop Media Index</strong><p>Extremely serious measurements of foid density, yearning, Tumblr residue, and other peer-reviewed metrics.</p></a></div><section class="culture-index-dictionary"><p class="content-eyebrow">Need a word explained?</p><h2>The Slop Dictionary</h2><p>Mog. Femoid. AI slop. Looksmaxxing. Vocabulary from places your browser should probably clear automatically.</p><a href="../dictionary">Open the dictionary</a></section></main>${footer(route)}`;
}
function renderSlopIndex() {
  ...
  return `${commonHead({ route, title: `${slopIndex.title} | foidslop`, description: slopIndex.description, schema })}${header(route, 'culture')}<main id="main" class="culture-index slop-index"><p class="content-eyebrow">Classification desk / peer review denied</p><h1>${esc(slopIndex.title)}</h1><p class="article-deck">${esc(slopIndex.description)}</p><div class="slop-index-grid">${slopIndex.items.map(item => `<article class="slop-index-card"><div class="slop-index-card-head"><span>${esc(item.type)}</span><strong>${esc(item.name)}</strong></div><dl><div><dt>Foid Density</dt><dd>${item.foidDensity}%</dd></div><div><dt>Yearning</dt><dd>${item.yearning}%</dd></div><div><dt>Male Understandability</dt><dd>${item.maleUnderstandability}%</dd></div><div><dt>Tumblr Residue</dt><dd>${esc(item.tumblrResidue)}</dd></div><div><dt>Slop Factor</dt><dd>${Number(item.slopFactor).toFixed(1)}</dd></div><div><dt>Actually Good</dt><dd>${esc(item.actuallyGood)}</dd></div></dl><p>${esc(item.verdict)}</p></article>`).join('')}</div><p class="slop-index-note">These scores are editorial jokes about media, not scientific claims about the people who enjoy it. Please do not submit this table to a journal.</p></main>${footer(route)}`;
}
