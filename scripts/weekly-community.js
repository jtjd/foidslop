...
function buildDispatchEmail(homepage, queue, mealsDb, poll, dateOverride = null) {
  ...
  const slopTrialPool = slopTrials.items.filter(item => item.kind === 'current' && item.image && fs.existsSync(path.join(ROOT, item.image.replace(/^\//, ''))) && (!item.activeFrom || item.activeFrom <= window.opensDate) && (!item.activeUntil || item.activeUntil >= window.opensDate));
